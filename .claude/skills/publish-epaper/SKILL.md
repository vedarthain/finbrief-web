...
5. **Announcements** — single-company news that is a statement, plan, or intention rather than a completed transaction or a personnel move: stated intentions/plans not yet executed ("eyes foray into X", "aims to chase Y", "mulls Z"), corporate statements/clarifications/responses to press queries. *(Grouped under "Corporate" top-level tab)*
6. **Appointments** — executive/leadership personnel moves at any notable company, domestic or international: CEO/MD/chairman/CXO-level appointments, resignations, promotions, successions, board reconstitutions. *(Grouped under "Corporate")*
7. **Events** — single-company news describing something that concretely happened/closed: results announced, M&A or stake-sale deals signed/closed, capex/capacity commissioned or expanded, regulatory clearance obtained for a completed deal, funding rounds closed, facility/JV launches — e.g. "SJVN capacity up 1,730 MW" or "Zollner enters India with Avalon JV" is a completed action, so it's Events, not Announcements. *(Grouped under "Corporate")*
8. **IPO** — new listings, IPO price bands/allotments/proceeds, rights issues, preferential issues. *(Grouped under "Stocks")*
9. **Market** — index/auction moves, currency, bank credit/liquidity, mutual/HNI fund flows, open offers, post-offer advertisements. *(Grouped under "Stocks")*
10. **Trade** — bilateral/multilateral trade deals, tariffs, trade missions, investment treaties. *(Grouped under "Stocks")*
11. **Insurance** — insurance-sector and insurer-specific stories. *(Grouped under "Stocks")*
...
**Note:** "Corporate Events" (singular combined section) no longer exists — it has been split into "Announcements", "Appointments", and "Events" above, all three grouped under the "Corporate" top-level tab (previously Announcements/Events were grouped under "Stocks").
...
- Render layer: `components/PaperTree.tsx` — "Top Stories" and "Market Impact" tabs (from `getTopStories`/`getMarketImpactStories`) shown as their own top-level ribbon tabs next to font-size/calendar controls, alongside "Stocks in Focus", then section-tree sidebar + click-to-expand row list (`components/PaperSectionTable.tsx`, numbered single-line-truncated rows); a search box filters headline+summary across every section; `is_notice` stories are hidden per-section behind a "Show N routine notices" toggle — all other stories (including low-`importance` ones) are shown by default, `importance` only affects sort order (§3d); `renderSummary()` parses `[[...]]` markers. `components/IpoTable.tsx` — structured IPO/listing table, separate page (`app/ipo/page.tsx`).
